...
def test_sidebar_shows_icons_and_triage_count(client_local, org):
    ...
    ws = Workspace.objects.get(org=org)  # TODO(task-5): pass org directly
    create_slice(get_or_create_triage(ws), "미분류 1")
    p = f"/{org.slug}/{ws.slug}"
...
def test_lens_count_context_processors(client_local, org):
    ...
    triage_slice = create_slice(get_or_create_triage(ws), "captured", status="building")
...
def test_sidebar_inbox_count_and_no_lens_tabs(client_local, org):
    ...
    from tuckit.core.services.slices import create_slice
    ws = Workspace.objects.get(org=org)  # TODO(task-5): pass org directly
    create_slice(get_or_create_triage(ws), "미분류", status="idea")
    p = f"/{org.slug}/{ws.slug}"
    body = client_local.get(f"{p}/").content.decode()
