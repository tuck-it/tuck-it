...
class Command(BaseCommand):
    help = "Create the default local user, org, area, and API token."

    def handle(self, *args, **options):
        org, raw = ensure_bootstrap()
        self.stdout.write(self.style.SUCCESS(f"Org ready: {org.slug}"))
        if raw:
            self.stdout.write(self.style.WARNING(f"API token (shown once): {raw}"))
        else:
            self.stdout.write("API token already exists — re-run with a fresh org to mint another.")
